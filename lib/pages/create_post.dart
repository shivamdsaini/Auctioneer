...
class _CreatePostState extends State<CreatePost> {
  ...
  Future pickimageCam() async{
   try{
    ...
    DateTime? pickedDateTime; // using pickedDate and pickedTime to generate equivalent dateTime
        ...
        body: SingleChildScrollView(
          child: Center(
            child: Column(
              ...
              children: [
                SizedBox(height: 10,),
               image != null ? ClipOval(child: Image.file(image!,width: 160,height: 160,fit: BoxFit.cover,)) : Icon(Icons.add_photo_alternate_outlined ,color: Colors.white,size:140),
                SizedBox(height: 10,),
                Row(
                  ...
                  children: [
                    GestureDetector(
                      onTap:()=>pickImage(),
                      child: Icon(Icons.image_outlined,color: Color.fromARGB(255, 189, 55, 100),size: 50,)
                      ),
                    SizedBox(width: 30),
                    GestureDetector(
                      onTap:()=>pickimageCam(),
                      child: Icon(Icons.camera_alt_outlined,color: Color.fromARGB(255, 84, 139, 175),size: 50,)
                      ),
                  ],
                )
                 ,
                    ...
                    if(pickedDate!=null){
                     pickedTime = await showTimePicker(
                      ...
                      builder: (context,child)=>Theme(
                       data: ThemeData().copyWith(
                        textTheme: TextTheme(
                           bodyText1: TextStyle(fontSize: 20.0), // <-- here you can do your font smaller
                  ...
                  child: GestureDetector(
                   onTap:() async {
                    ...
                    const snackBar = SnackBar(
                      ...
                      const snackBar = SnackBar(
                         content: Text('Please add a description for your Item!'),
                        //  backgroundColor: Colors.grey[200],
                       );
                     ...
                     const snackBar = SnackBar(
                      ...
                      const snackBar = SnackBar(
                         content: Text('Please! select date and time for the auction!'),
                        //  backgroundColor: Colors.grey[200],
                       );
                      ...
                      var bytes = image!.readAsBytesSync(); // toconvert imageFile to bytes
                     var headers = {
                      'Authorization': 'Client-ID a46304488bff32c'
                     };
                     var request = http.MultipartRequest('POST', Uri.parse('https://api.imgur.com/3/image'));
                     request.fields.addAll({
                      'image': base64Encode(bytes), // encoding bytes in base64 string for uploading
                     });
...
Widget buildUserCurr(User user)=> (user!=null && user.Datetime.isBefore(DateTime.now()) && user.Datetime.isAfter(DateTime.now().subtract(Duration(minutes: 30)))) ? ItemCard(user:user) : Container() ;

Widget buildUserPast(User user)=> (user.Datetime.isBefore(DateTime.now().subtract(Duration(minutes: 30)))) ? Card(
                          ...
                          child: Column(
                            children: [
                              ...
                              Container(
                                      ...
                                      child: Text(
                                        user.bidder!=""? 'Sold for ₹${user.bidprice} to ${user.bidder}':"item not sold",
                                        style: TextStyle(
                                           // fontFamily: 'sans-serif',
                                            fontWeight: FontWeight.bold),
                                      ),
...
Widget buildUserFut(User user)=> (user.Datetime.isAfter(DateTime.now())) ? Card(
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(20),
                        ),
                        margin: EdgeInsets.all(30),
